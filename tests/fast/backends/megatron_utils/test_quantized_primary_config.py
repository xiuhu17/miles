...
from miles.backends.megatron_utils.model_provider import (
    _apply_bridge_fp8_runtime_config,
    configure_lora_primary_weight_storage,
)
...
def test_bridge_fp8_runtime_config_enables_mxfp8_primary_storage():
    ...
    args = _fp8_args()
    configure_lora_primary_weight_storage(args, role="actor")
    ...
    assert provider.fp8_backward_override == "dequantized"
    assert provider.omit_columnwise_primary_weight_storage is True
...
def test_bridge_row_only_lora_requires_new_megatron_config_field():
    provider = _fp8_provider()
    del provider.omit_columnwise_primary_weight_storage
    args = _fp8_args()
    configure_lora_primary_weight_storage(args, role="actor")

    with pytest.raises(RuntimeError, match="does not expose omit_columnwise"):
        _apply_bridge_fp8_runtime_config(provider, args)


def test_full_ft_never_omits_columnwise_primary_storage():
    args = _fp8_args(lora_rank=0)

    assert configure_lora_primary_weight_storage(args, role="actor") is False
    assert args.omit_columnwise_primary_weight_storage is False


def test_non_actor_lora_never_omits_columnwise_primary_storage():
    args = _fp8_args()

    assert configure_lora_primary_weight_storage(args, role="critic") is False
    assert args.omit_columnwise_primary_weight_storage is False
...
class _FakeParam:
    def __init__(
        ...
        *,
        requires_grad: bool,
        init_value: torch.Tensor | None,
        format_name: str = "BF16",
    ):
...
class _FakeModel:
    ...
    def named_parameters(self):
        return iter((f"param_{index}", param) for index, param in enumerate(self._params))
...
def test_row_only_mxfp8_contract_accepts_only_frozen_primary_weights(monkeypatch):
    monkeypatch.setattr(
        "miles.backends.megatron_utils.bridge_lora_helpers.native_format_name",
        lambda param: param.format_name,
    )
    frozen = _FakeParam(requires_grad=False, init_value=None, format_name="MXFP8")

    assert _assert_row_only_mxfp8_primary_weights_are_frozen([_FakeModel([frozen])]) == 1

    trainable = _FakeParam(requires_grad=True, init_value=None, format_name="MXFP8")
    with pytest.raises(RuntimeError, match="trainable quantized parameters"):
        _assert_row_only_mxfp8_primary_weights_are_frozen([_FakeModel([trainable])])
